...
* The tenant network has the default gateway.  In this case,
  the address can be forwarded to the default gateway.  This implementation
  can handle requests even if their L2 destination is the default gateway.

Typically adding link-local routes for 169.254.169.254 is enough.
For example,
<pre>
